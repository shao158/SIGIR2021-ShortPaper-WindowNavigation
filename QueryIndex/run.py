...
random_bmw_size4 = " /share/msmarco-passage/variable-bmw/bm25L/bm25L.sizes "
    ...
    for p in topk:
        for s in score:
            for b in blocksize:
                ...
                if(score == " BM25"):
                    # if(x == " NV"):
                    subprocess.run(command4+i+b+p+s+" invalidate_address",shell=True,stdout=f)
                    # else:
            ...
            subprocess.run(command4+i+" 0"+p+s+random_bmw_size3, shell=True,stdout=f_1)
